...
function AzBlobWritable (saslocator, fileName)  {
  if (!(this instanceof AzBlobWritable)) {
    return new AzBlobWritable(storageacc, container, sas, fileName)
  } else {
    ...
    FlushWritable.call(this, {highWaterMark: (2*1024*1024)}); // 1MB
  }
}  
...
AzBlobWritable.prototype._write = function(source, enc, next) {
  ...
  if (this.blockBuffer_length < BLOCK_SIZE) { 
    ...
    // blockBuffer
     this._putblock (this.currblock, Buffer.from(this.blockBuffer, 0, this.blockBuffer_length)).then(() => {
       next() // send next data (add a error string if error)
     }, (err) => next(err))
     this.blockBuffer_length = 0
     this.currblock++
     if (source.length > source_to_copy) {
       // got remainder to copy
       source.copy(this.blockBuffer, this.blockBuffer_length, source_to_copy)
       this.blockBuffer_length = source.length - source_to_copy
     }
  }
}
...
AzBlobWritable.prototype._flush = function(next) {
  ...
  // The 'end' event is emitted after all data has been output, which occurs after the callback in _flush() has been called.
    ...
    if (this.blockBuffer_length >0) {
      this._putblock (this.currblock, Buffer.from(this.blockBuffer, 0, this.blockBuffer_length)).then(finalBlockFn, (err) => next(err))
    } else {
      finalBlockFn()
    }
}
